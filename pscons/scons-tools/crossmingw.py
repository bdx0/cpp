...
def shlib_emitter(target, source, env):
    ...
    if not dll:
        raise SCons.Errors.UserError, "A shared library should have \
        exactly one target with the suffix: %s" % env.subst("$SHLIBSUFFIX")
...
def generate(env):
    ...
    env[
        'RCINCFLAGS'] = '$( ${_concat(RCINCPREFIX, CPPPATH, \
        RCINCSUFFIX, __env__, RDirs, TARGET)} $)'
    env['RCINCPREFIX'] = '--include-dir '
    env['RCINCSUFFIX'] = ''
    env['RCCOM'] = '$RC $RCINCFLAGS $RCINCPREFIX \
        $SOURCE.dir $RCFLAGS -i $SOURCE -o $TARGET'
